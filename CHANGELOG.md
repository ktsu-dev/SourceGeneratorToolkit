## v1.1.0 (minor)

Changes since v1.0.0:

- Reject a reused diagnostic number in DiagnosticCatalog ([@Claude](https://github.com/Claude))
- Gate Dependabot auto-merge on CI actually being green ([@Claude](https://github.com/Claude))
- test: cover the InvalidOperationException path in Deserialize [patch] ([@Claude](https://github.com/Claude))
- fix: report a diagnostic for metadata shapes System.Text.Json cannot construct [patch] ([@Claude](https://github.com/Claude))
- ci: adopt the consolidated .NET workflow [patch] ([@Claude](https://github.com/Claude))

## v1.0.11 (patch)

Changes since v1.0.10:

- Bump Polyfill from 11.3.0 to 11.4.0 ([@dependabot[bot]](https://github.com/dependabot[bot]))
- Bump the ktsu group with 10 updates ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.10 (patch)

Changes since v1.0.9:

- Bump the ktsu group with 10 updates ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.9 (patch)

Changes since v1.0.8:

- Bump the ktsu group with 1 update ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.8 (patch)

Changes since v1.0.7:

- Bump ktsu.CodeBlocker and 9 others ([@dependabot[bot]](https://github.com/dependabot[bot]))
- Bump MSTest.Sdk from 4.4.0 to 4.4.1 ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.7 (patch)

Changes since v1.0.6:

- Gate Dependabot auto-merge on CI actually being green ([@Claude](https://github.com/Claude))

## v1.0.6 (patch)

Changes since v1.0.5:

- Bump the ktsu group with 1 update ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.5 (patch)

Changes since v1.0.4:

- Bump the ktsu group with 10 updates ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.4 (patch)

Changes since v1.0.3:

- test: cover the InvalidOperationException path in Deserialize [patch] ([@Claude](https://github.com/Claude))
- fix: report a diagnostic for metadata shapes System.Text.Json cannot construct [patch] ([@Claude](https://github.com/Claude))

## v1.0.3 (patch)

Changes since v1.0.2:

- ci: adopt the consolidated .NET workflow [patch] ([@Claude](https://github.com/Claude))

## v1.0.2 (patch)

Changes since v1.0.1:

- Bump Polyfill from 11.2.0 to 11.3.0 ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.1 (patch)

Changes since v1.0.0:

- Bump the system group with 2 updates ([@dependabot[bot]](https://github.com/dependabot[bot]))

## v1.0.0 (major)

- feat: extract the reusable source-generator infrastructure from Semantics [minor] ([@matt-edmondson](https://github.com/matt-edmondson))

